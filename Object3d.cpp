...
void Object3d::LoadTexture(const std::string& directoryPath, const std::string& filename)
{
	...
	//ユニコード文字列に変換する
	wchar_t wifepath[128];
	int iBufferSize = MultiByteToWideChar(CP_ACP, 0, filepath.c_str(), -1, wifepath, _countof(wifepath));

	// WICテクスチャのロード
	//result = LoadFromWICFile(L"Resources/tex1.png", WIC_FLAGS_NONE, &metadata, scratchImg);
	result = LoadFromWICFile(wifepath, WIC_FLAGS_NONE, &metadata, scratchImg);
	assert(SUCCEEDED(result));
...
void Object3d::CreateModel()
{
	...
	while (getline(file, line)) {
		...
		if (key == "f")
		{
			...
			{
				//頂点インデックス1個分の文字列をストリームに変換して解析しやすく
